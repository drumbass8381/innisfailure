...
import { getWatchers, getTimeframe } from "@innisfailures/processing";
import { decomposeSymbolId } from "@innisfailures/tools";
import { BarSize, ExchangeCode } from "@innisfailures/types";
  ...
  cleanStaleChannels(bots: TBotWithExchangeAccount[]) {
    ...
    for (const bot of bots) {
      ...
      botsInUse.push({
        ...
        symbols: watchTrades,
        exchangeCodes: [...new Set(watchTrades.map((symbolId) => decomposeSymbolId(symbolId).exchangeCode))],
      });
    }
