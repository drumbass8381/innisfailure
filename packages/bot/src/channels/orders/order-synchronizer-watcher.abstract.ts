...
import type { ExchangeAccountWithCredentials } from "@innisfailures/db";
import { logger } from "@innisfailures/logger";
import type { Subscription, OrderEventType } from "./types.js";
...
export abstract class OrderSynchronizerWatcher {
   ...
   */
  subscribe(event: OrderEventType, callback: Subscription["callback"]) {
    this.consumers.push({
      event,
      callback,
    });
  ...
  unsubscribe(event: OrderEventType, callback: Subscription["callback"]) {
  ...
  protected emit(event: OrderEventType, args: Parameters<Subscription["callback"]>) {
