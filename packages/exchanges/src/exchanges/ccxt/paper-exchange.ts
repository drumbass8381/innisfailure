...
export class PaperExchange extends CCXTExchange {
      ...
      try {
        ...
        for (const order of this.openOrders) {
          ...
          if (order.side === "buy" && order.price! >= ticker.ask!) {
            const filledOrder = await xprisma.$transaction(async (tx) => {
              const updated = await tx.paperOrder.update({
                where: { id: order.id },
                data: {
              ...
              await applyCexPaperFillLedger(tx, {
                symbol: order.symbol,
                side: "buy",
  ...
  async placeMarketOrder(params: IPlaceMarketOrderRequest): Promise<IPlaceMarketOrderResponse> {
    ...
    const filledPrice = (params.side === "buy" ? ticker.ask : ticker.bid) as number;
    const filledOrder = await xprisma.$transaction(async (tx) => {
      const updated = await tx.paperOrder.update({
        where: { id: order.id },
        data: {
