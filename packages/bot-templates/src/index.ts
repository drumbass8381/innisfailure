 ...
 */
import type { BotTemplate } from "@innisfailures/bot-processor";
...
export function findTemplate(template: string): BotTemplate<any> {
  ...
  throw new Error(
    `Template ${template} not found. Ensure that the "${template}.ts" file exists in @innisfailures/bot-templates`,
  );
}
