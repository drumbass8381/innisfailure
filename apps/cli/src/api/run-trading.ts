...
import { ExchangeAccountWithCredentials, xprisma } from "@innisfailures/db";
import { getTServer } from "@innisfailures/daemon";
...
export async function runTrading(strategyName: string, options: Options): Promise<CommandResult> {
  ...
  if (bot?.enabled) {
    ...
    const server = await getTServer();
    await server.bot.stop({ botId: bot.id });
