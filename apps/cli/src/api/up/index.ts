...
export async function up(options: Options): Promise<CommandResult> {
  ...
  if (pid) {
    logger.warn(`innisfailures already running [PID: ${pid}]`);
  ...
  if (daemonProcess.pid === undefined) {
    throw new Error("innisfailures process not started. PID is undefined.");
  }

  logger.debug(`innisfailures daemon started with PID: ${daemonProcess.pid}`);

  if (options.detach) {
    daemonProcess.unref();
    savePid(daemonProcess.pid);
    logger.info(`innisfailures started as a daemon [PID: ${daemonProcess.pid}]`);
  } else {
