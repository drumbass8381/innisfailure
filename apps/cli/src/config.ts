...
export function readExchangesConfig(
  ...
  const raw = JSON5.parse<Record<string, Partial<ExchangeConfig> & Pick<ExchangeConfig, "exchangeCode">>>(
    fs.readFileSync(configPath, "utf8"),
  );

  const config: Record<string, ExchangeConfig> = {};
  for (const [label, entry] of Object.entries(raw)) {
    config[label] = {
      ...
      isDemoAccount: entry.isDemoAccount ?? false,
      isPaperAccount: entry.isPaperAccount ?? false,
    };
  }
